...
**Trigger phrase:** `run security`  
**Receives:** Full codebase, all prior agent outputs  
**Produces:** Security report — CRITICAL / WARNING / INFO / PASS  
**Recommended model:** `claude-sonnet-4-6` — security reasoning requires judgment, not just pattern matching — escalate to `claude-fable-5` for large codebases, authentication systems, or any review where a missed vulnerability would be catastrophic
...
Work through `checklists/security-checklist.md` section by section.
For each item: pass, fail (CRITICAL/WARNING), or N/A.
